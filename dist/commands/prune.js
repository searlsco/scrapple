...
export async function prune(options, global) {
    const result = pruneRawSamples(options);
    if (global.human) {
        const action = result.dryRun ? 'Would delete' : 'Deleted';
        console.log(`${action} ${result.filesDeleted.toLocaleString()} raw sample archives ` +
...
export function pruneRawSamples(options = {}) {
