 ...
 */
export function buildBreadcrumbs(url) {
 ...
 */
export function resolveReference(ref, db) {
    // Try as ID first (16 hex chars)
    if (/^[a-f0-9]{16}$/.test(ref)) {
        return db.prepare('SELECT * FROM manifest WHERE id = ?').get(ref);
    }
    ...
    let manifest = db.prepare('SELECT * FROM manifest WHERE url = ?').get(url);
    if (manifest)
        return manifest;
    // Try case-insensitive match (Apple URLs have inconsistent casing)
    manifest = db.prepare('SELECT * FROM manifest WHERE LOWER(url) = LOWER(?)').get(url);
    return manifest;
}
export async function show(ref, _options, global) {
    const db = getDb();
