...
function withDb(test: (db: Database.Database) => Promise<void> | void): () => Promise<void> {
  return async () => {
...
function insertDoc(db: Database.Database, url: string, title = 'Doc'): void {
  db.prepare(`
...
describe('discoverDocGraph', () => {
  it('persists processed docs and skips them on later runs', withDb(async (db) => {
    ...
    try {
      ...
      const progress = db.prepare('SELECT status, refs_found, processed_at FROM doc_graph_progress WHERE url = ?').get(sourceUrl) as {
        status: string
        refs_found: number
        processed_at: number
      }
  ...
  it('inserts linked documentation discovered from references', withDb(async (db) => {
    ...
    try {
      ...
      const linked = db.prepare('SELECT title, source FROM manifest WHERE url = ?').get(linkedUrl) as { title: string; source: string }
