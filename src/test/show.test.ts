...
describe('resolveReference', () => {
  ...
  function resolveReference(ref: string): { id: string; url: string } | undefined {
    // Try as ID first (16 hex chars)
    if (/^[a-f0-9]{16}$/.test(ref)) {
      return db.prepare('SELECT id, url FROM manifest WHERE id = ?').get(ref) as { id: string; url: string } | undefined
    }
    ...
    let manifest = db.prepare('SELECT id, url FROM manifest WHERE url = ?').get(url) as { id: string; url: string } | undefined
    if (manifest) return manifest

    // Try case-insensitive match
    manifest = db.prepare('SELECT id, url FROM manifest WHERE LOWER(url) = LOWER(?)').get(url) as { id: string; url: string } | undefined
    return manifest
  }
